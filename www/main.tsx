...
function createGithubIssueUrl(trace: Trace, url: string): string {
  const { version, target, trace_str } = ROUTE.exec(url).pathname.groups;
...
function Stacktrace({ trace, url }: { trace: Trace; url: string }) {
  return (
    <div class="bg-white shadow rounded-lg overflow-hidden">
      <div class="border-b border-gray-200 bg-gray-50 px-4 py-3">
        <div class="flex justify-between items-center">
          ...
          <a
            href={createGithubIssueUrl(trace, url)}
            target="_blank"
...
Deno.serve(async (req) => {
  ...
  if (res.value) {
    return render(<Stacktrace trace={res.value} url={req.url} />);
  }
  ...
  return render(<Stacktrace trace={trace} url={req.url} />);
});
